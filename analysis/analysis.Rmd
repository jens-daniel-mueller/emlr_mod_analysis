...
params_local     <-
  read_rds(paste("/home/donzhu/Thesis/emlr_cant/model/emlr_mod_",
        Version_ID,
        "/data/auxillary/",
        "params_local.rds",
        sep = ""))
...
GLODAP <- GLODAP %>%
  select(lon, lat, depth, era, basin_AIP, basin, gamma, gamma_slab, sal, temp, aou, nitrate, silicate, phosphate, phosphate_star, cstar_tref)
    ...
    for (i_era in unique(GLODAP$era)) {
      ...
      for (i_gamma_slab in unique(GLODAP_basin_era$gamma_slab)) {
        ...
        for (i_eras in unique(lm_best_basin_era_slab$eras)) {
          ...
          cstar_tref <- full_join(GLODAP_basin_era_slab, lm_best_basin_era_slab_eras)
          
          for (i_predictors in unique(lm_best_basin_era_slab_eras$predictors)) {
            i_predictors <- unique(lm_best_basin_era_slab$predictors)[1]
            print(i_predictors)
            
            cstar_tref1 <- b_cstar_tref(cstar_tref) %>%
              mutate(cstar_tref_residual = cstar_tref - cstar_tref_fitted)
            
              
              
              residual <-
              ggplot(cstar_tref1, aes(lat, cstar_tref_residual)) +
              geom_bin2d(binwidth = 1) +
              scale_fill_viridis_c(option = "magma",
                       direction = -1,
                       trans = "log10") +
              facet_wrap( ~ predictors, ncol = 2) +
                labs(x = expression(latitude~(degree*N)), y = expression(C[t[ref]]^"*"~residual),
